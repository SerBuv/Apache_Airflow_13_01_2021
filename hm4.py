...
args = {
    ...
    'start_date': dt.datetime(2021, 2, 10),  # Время начала выполнения пайплайна
...
def taskflow_api():
    ...
    def pivot_to_db(result_json: dict):
        ...
        for row in df.itertuples(index=False):
            db_query = 'insert into ' + tbl_name + ' (sex, c1_count, c2_count, c3_count) values (%s, %s, %s, %s)'
            pg_hook.run(db_query, parameters=tuple(row))
        return None
